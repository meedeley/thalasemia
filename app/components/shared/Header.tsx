...
import { Menu, Heart, Youtube, Home, Info, BookOpen } from "lucide-react";
...
export default function Header() {
  ...
  const navLinks = [
    { id: "hero", label: "Beranda", path: "/", icon: <Home className="w-5 h-5 mr-2" /> },
    { id: "about", label: "Panduan Orang Tua", path: "/parent", icon: <Info className="w-5 h-5 mr-2" /> },
    { id: "cheerful-story", label: "Dongeng Ceria", path: "/children", icon: <BookOpen className="w-5 h-5 mr-2" /> },
  ];

  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 pt-4 px-4 lg:px-8 transition-all duration-500 ${
        isScrollingDown ? "opacity-0 -translate-y-full" : "opacity-100 translate-y-0"
      }`}
    >
      <div
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => handleNavigation("hero")}>
            <div className={`${isScrolled ? "w-24 h-24" : "w-32 h-32 lg:w-48 lg:h-48"} flex items-center justify-center`}>
              <img src="/logo.png" alt="Logo Thalasemia" className="w-full h-full object-contain transition-all" />
            </div>
          </div>
          ...
          <nav className="hidden md:flex flex-1 justify-center items-center gap-2">
            {navLinks.map((link) => (
              <Button
                ...
                className={`text-gray-700 hover:text-pink-600 hover:bg-white/70 transition-all font-medium ${
                  isScrolled ? "text-xs px-2 h-8" : ""
                }`}
              >
                {link.label}
              </Button>
          ...
          {/* Desktop Action */}
          <div className={`hidden md:flex items-center ${isScrolled ? "gap-1" : "gap-2"}`}>
            <Button
          ...
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            <SheetTrigger asChild>
              <Button variant="ghost" size="icon" className="md:hidden hover:bg-pink-50 transition-all">
              ...
              <div className="flex flex-col gap-2 p-6">
                {navLinks.map((link, index) => (
                  <Button
                    ...
                    variant="ghost"
                    onClick={() => handleNavigation(link.id, link.path)}
                    className="justify-start text-base text-gray-700 hover:text-pink-600 hover:bg-pink-50 transition-all h-12 rounded-lg flex items-center"
                    style={{
